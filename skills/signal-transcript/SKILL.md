...
description: "Pull a scoped markdown transcript from the user's Signal Desktop database when they reference a Signal conversation, instead of asking them to paste messages"
---

# signal-transcript — read the user's Signal history directly

When the user references a Signal conversation ("what did Sam say", "check my thread with X", "look at the last 10 minutes with Y"), run this instead of asking them to paste messages in.

Run `signal-transcript --help` for full usage — it is the authoritative reference.

Rules for Claude:

- Primary invocation: `signal-transcript "Sam" --since 10m`. Output is markdown on stdout — read it directly; only use `--out FILE` for unusually large pulls.
- **Following a live conversation**: every run ends with a `_cursor: N_` line. Seed with one windowed pull for context (`--since 30m`), then poll with `signal-transcript "Sam" --after N`, taking the new cursor from each result. `--after` returns only what arrived since, so nothing you have already read is re-emitted; when nothing is new it costs a single line. Never re-poll with `--since` — that re-renders the whole window into context every time.
...
- Scope discipline: this reads the user's private messages. Always scope to the named person and the smallest sensible time window (`--since`, `--last N`). Never dump the whole archive, and don't pull unrelated conversations out of curiosity.
...
- Ambiguous name match exits 2 and lists candidates on stderr — pick from those or ask the user, don't retry with a guessed name.
...
- If stderr shows a `Signal schema changed` warning or error, a Signal Desktop update moved something — tell the user rather than silently handing back a possibly-degraded transcript.
- macOS: the DB key comes from the Keychain. Normal runs are silent (the key is cached in a `signal-transcript` Keychain item), but when the cache is absent or stale (first run, Signal re-key, uv Python upgrade) macOS pops **two** permission dialogs — Signal's item is double-gated, so this is expected; warn the user before running in that state, and never run it when they're away from the screen if a dialog could appear (it would hang until clicked).
